    ...
    <>
      <div className="page-hero" style={{ background: 'linear-gradient(135deg, #78350f 0%, #b45309 60%, #d97706 100%)' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <p className="text-amber-100 text-lg max-w-2xl ph-subtitle">Premium quality cashew kernels in all internationally traded grades — whole, halves, splits, and pieces. Sourced and processed in India.</p>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {applications.map((a, i) => (
              <ScrollReveal key={a.name} animation="bubble" delay={i * 80}>
                <div className="bg-amber-50 border border-amber-100 rounded-2xl p-5 text-center market-card h-full">
                  <div className="text-3xl mb-3 market-emoji">{a.emoji}</div>
            ...
            <ScrollReveal animation="left">
              <div>
                ...
                <ul className="space-y-2 text-amber-100 text-sm">
                  {['10kg & 25kg vacuum-sealed bags', 'Retail-ready pouch options (private label)', 'Container loads (20ft & 40ft)', 'Custom labelling & barcode support', 'Export documentation included'].map((item, i) => (
                    <ScrollReveal key={item} animation="left" delay={i * 70 + 100}>
                      <li className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-amber-300 flex-shrink-0" />{item}</li>
                    </ScrollReveal>
                  ))}
                </ul>
              </div>
